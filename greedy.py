...
"""Fast SI, Greedy grouping, and ICS reporting for GFlow-VQE systems."""
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            ...
            "for Jordan-Wigner systems in gflow_vqe.hamiltonians."
        )
    )
    parser.add_argument(
        "func_name",
...
def main(argv=None):
    ...
    print(
        "Building {} Jordan-Wigner Hamiltonian...".format(args.func_name),
        flush=True,
    )
    (
        molecule,
    ...
    print(
        "{} Energy={:.16g}".format(args.wfn, reference_energy),
        flush=True,
    )
    ...
    if args.wfn == "FCI":
        ...
        print(
            "FCI Energy={:.16g}".format(
                clean_real(fci_energy, tiny=1.0e-7)
            ),
            flush=True,
        )
    ...
    for result in results:
        if args.wfn == "FCI":
            ...
            fci_variances = direct_group_variances(
                result.groups,
                fci_state,
                n_qubits,
            )
            result.fci_eps_sq_m = eps_sq_m_from_variances(fci_variances)
