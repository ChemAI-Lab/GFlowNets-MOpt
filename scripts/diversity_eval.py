...
def main(argv=None):
    ...
    compute_ics_metric(x_no, context)
    for idx, candidate in enumerate(ics_candidates, start=1):
        compute_ics_metric(candidate, context)
        if idx % 10 == 0 or idx == len(ics_candidates):
            print("Computed ICS for {}/{} candidates.".format(idx, len(ics_candidates)))
