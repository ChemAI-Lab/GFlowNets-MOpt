"""Fast Pareto metrics for loaded standard and coefficient-ordered graphs.

The serialized ``FermionOperator`` is Bravyi--Kitaev transformed exactly as
in the loaded driver and its exact ground-state vector is used for the
measurement metric. Saved graphs are mapped through each node's embedded
``v`` metadata, so native and coefficient-sorted node orders are both valid.
...
from openfermion import FermionOperator, count_qubits, get_sparse_operator
...
HAM_LIBRARY_DIRECTORY = Path(__file__).resolve().parents[1] / "ham_lib"
TRANSFORMATION = "bk"
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Fast Pareto metrics for standard or coefficient-ordered sampled "
            "graphs from serialized loaded Hamiltonians."
        )
    )
    parser.add_argument(
        "molecule",
        ...
        metavar="MOLECULE",
        help="Loaded molecule name: {}.".format(", ".join(LOADED_SYSTEM_NAMES)),
    )
    parser.add_argument(
        "--fig-name",
        default=None,
        help=(
            "Artifact filename stem. The default is the canonical molecule "
            "name used by the loaded ordered driver (for example LiH)."
        ),
    )
    parser.add_argument(
        "--tf",
        ...
        help="Fermion-to-qubit transformation (default: bk).",
    )
    parser.add_argument(
        "--ham-lib-dir",
        type=Path,
        default=HAM_LIBRARY_DIRECTORY,
        help=(
            "Directory containing <molecule>_fer.bin "
            "(default: repository ham_lib directory)."
        ),
    )
    parser.add_argument(
        "--y-axis",
    ...
    if args.fig_name is None:
        args.fig_name = DISPLAY_NAMES[args.molecule]
    return args
...
def load_qubit_hamiltonian(
    ...
    """Load a serialized FermionOperator and apply the loaded driver's BK map."""
    ...
    if str(transformation).lower() != TRANSFORMATION:
        raise ValueError(
            "Transformation '{}' is unsupported; the loaded driver uses BK.".format(
                transformation
            )
        )

    input_path = Path(library_directory).expanduser() / "{}_fer.bin".format(
        molecule
    )
    ...
    return bravyi_kitaev(fermion_hamiltonian), input_path
...
def build_metrics_fast(records, qubit_operator, workers):
    ...
    print(
        "Loaded Hamiltonian and exact FCI wavefunction: {:.3f} s".format(
            time.perf_counter() - setup_start,
        )
    )
    print("FCI Energy={}".format(energy))
    print("Number of Qubits={}".format(n_qubits))

    terms = make_pauli_terms(qubit_operator, n_qubits)
    print("Number of Pauli products to measure: {}".format(len(terms)))
...
def main(argv=None):
    ...
    if metrics is None:
        qubit_operator, input_path = load_qubit_hamiltonian(
            args.molecule,
            transformation=args.tf,
            library_directory=args.ham_lib_dir,
        )
        print("Loaded Hamiltonian from {}".format(input_path))
        build_start = time.perf_counter()
        metrics = build_metrics_fast(records, qubit_operator, args.workers)
