"""Ordered-graph counterpart of GFlowICS_fast.py.

This variant resolves saved GFlowNet graph nodes through their embedded Pauli
term metadata instead of assuming native BinaryHamiltonian node order.
"""
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Compare Tequila ICS against ICS initialized from coefficient-ordered "
            "GFlowNet-compatible groups."
        )
    )
    parser.add_argument(
        "func_name",
    ...
    return tuple(
        sorted(
            (
                int(node),
                data.get("color"),
                _pauli_support_key(data["v"]) if "v" in data else None,
            )
            for node, data in graph.nodes(data=True)
        )
    )
...
def main(argv=None):
    ...
    gflow_initial_groups = groups_from_ordered_gflow_graph(
        binary_hamiltonian,
        best_gflow_graph,
        condition="fc",
    )
    ...
    gflow_ics_groups, gflow_ics_sample_size = iterative_coefficient_splitting_from_ordered_gflow_graph(
        binary_hamiltonian,
        best_gflow_graph,
        cov_dict,
        condition="fc",
