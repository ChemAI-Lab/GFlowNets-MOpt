"""Scalable SI/ICS analysis for coefficient-ordered loaded Hamiltonians.

The serialized ``ham_lib/*_fer.bin`` inputs are explicitly Bravyi-Kitaev
transformed before analysis and evaluated with their exact FCI state.  The v2
path keeps the standard Tequila sorted-insertion partition, resolves ordered
graph nodes through their embedded Pauli metadata, plans exactly the covariance
pairs that SI/ICS can query, and evaluates only those moments.  It also
deduplicates equivalent graph partitions and can process independent ICS
...
DEFAULT_HAM_LIBRARY_DIRECTORY = Path(__file__).resolve().parents[1] / "ham_lib"
TRANSFORMATION = "BK"
    ...
    for node, data in graph.nodes(data=True):
        ...
        if "v" not in data:
            raise ValueError(
                ...
                "Regenerate it with the coefficient-ordered loaded driver.".format(node)
            )
...
def hamiltonian_state_fingerprint(binary_terms, state_vector, n_qubits, wfn_method):
    digest = hashlib.sha256()
    digest.update(
        "pareto-fast-loaded-bk-v{}".format(V2_CACHE_VERSION).encode("ascii")
    )
...
def _cache_path(cache_dir, func_name, wfn_method, suffix):
    # ``--fig-name`` may include a directory.  Cache entries still belong
    # under ``--cache-dir`` rather than inheriting that absolute/relative path.
    result_stem = Path(str(func_name)).name
    safe_name = "{}_{}_{}".format(result_stem, str(wfn_method).lower(), suffix)
...
def load_loaded_hamiltonian(molecule, library_directory):
    """Load a serialized FermionOperator and apply the fixed BK mapping."""

    input_path = Path(library_directory).expanduser() / "{}_fer.bin".format(molecule)
    if not input_path.is_file():
        raise FileNotFoundError(
            "Could not find loaded Hamiltonian '{}'.".format(input_path)
        )
    ...
    n_paulis = sum(bool(pauli_tuple) for pauli_tuple in qubit_hamiltonian.terms)
    if n_paulis == 0:
        raise ValueError("The loaded Hamiltonian has no measurable Pauli terms.")
    return input_path, tequila_hamiltonian, qubit_hamiltonian, n_paulis


def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Pareto plot for coefficient-ordered graphs generated from a "
            "serialized Bravyi-Kitaev Hamiltonian, with sorted-insertion and "
            "optional ICS overlays."
        )
    )
    parser.add_argument(
        "molecule",
        ...
        metavar="MOLECULE",
        help="Loaded molecule name: {}.".format(", ".join(AVAILABLE_MOLECULES)),
    )
    parser.add_argument(
        "--fig-name",
        default=None,
        help=(
            "Sampled-graph/result filename stem. By default the canonical "
            "molecule name is used (for example LiH)."
        ),
    )
    parser.add_argument(
        "--ham-lib-dir",
        type=Path,
        default=DEFAULT_HAM_LIBRARY_DIRECTORY,
        help=(
            "Directory containing *_fer.bin inputs (default: the repository's "
            "ham_lib directory)."
        ),
    )
    parser.add_argument(
        ...
        choices=(REFERENCE_METHOD,),
        help=(
            "Wavefunction used for SI/ICS evaluation. Loaded ordered results "
            "use exact FCI (default: FCI)."
        ),
    )
    parser.add_argument(
        "--cov-workers",
    ...
    args.fig_name = args.fig_name or DISPLAY_NAMES[args.molecule]
...
def main(argv=None):
    ...
    input_path, H, Hq, n_paulis = load_loaded_hamiltonian(
        args.molecule,
        args.ham_lib_dir,
    )
    ...
    print(
        "Loaded Hamiltonian: source={}, transformation={}, qubits={}, "
        "measurable Pauli terms={}, build_s={:.3f}.".format(
            input_path,
            TRANSFORMATION,
            n_qubits,
            n_paulis,
    ...
    state_gib = state_dimension * np.dtype(np.complex128).itemsize / 2**30
    print(
        "Preparing exact FCI state: Hilbert dimension={}, dense state={:.3f} GiB.".format(
            state_dimension,
            state_gib,
        ),
        flush=True,
    ...
    if n_qubits >= 20:
        print(
            "Resource warning: exact sparse FCI at {} qubits can require much more "
            "memory than the {:.3f} GiB statevector itself.".format(
                n_qubits,
                state_gib,
            ),
            flush=True,
    ...
    if args.cov_backend == "legacy":
        ...
        print(
            "Resource warning: the legacy covariance backend's Pauli-action "
            "matrix alone is approximately {:.3f} GiB.".format(action_gib),
            flush=True,
        )
    sparse_hamiltonian = get_sparse_operator(Hq, n_qubits=n_qubits)
    fci_energy, variance_wfn = get_ground_state(sparse_hamiltonian)
    del sparse_hamiltonian
    variance_wfn = np.asarray(variance_wfn, dtype=complex).reshape(-1)
    print(
        "Sparse Hamiltonian and {} state built in {:.3f} s; Energy={:.16g}.".format(
            REFERENCE_METHOD,
            time.perf_counter() - state_start,
            float(np.real_if_close(fci_energy)),
        ),
        flush=True,
    )
    ...
    if cache_dir:
        moment_cache_path = _cache_path(
            ...
            fig_name,
            args.wfn,
            "loaded_bk_moments_v2.p",
        )
        ics_cache_path = _cache_path(
            cache_dir,
            fig_name,
            args.wfn,
            "loaded_bk_{}_{}_ics_metrics_v2.p".format(
                grouping_condition,
                args.cov_backend,
            ),
        )
    ics_fingerprint = hashlib.sha256(
        "{}|loaded_bk|{}|{}|ics|sparse-tequila-report-v1".format(
            fingerprint,
            grouping_condition,
            args.cov_backend,
        ).encode("ascii")
